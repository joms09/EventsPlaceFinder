...
class EventParty : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ref = FirebaseDatabase.getInstance().getReference("event").orderByChild("type").equalTo("Party")
    ...
    fun firebaseData() {
        ...
        val firebaseRecyclerAdapter = object : FirebaseRecyclerAdapter<Model, MyViewHolder>(option) {
            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
                val itemView = LayoutInflater.from(this@EventParty).inflate(R.layout.card_view, parent, false)
            ...
            override fun onBindViewHolder(holder: MyViewHolder, position: Int, model: Model) {
                ...
                holder.img_vet.setOnClickListener {
                    ...
                    event.child("count").setValue(model.Count + 1).addOnCompleteListener {
                        val intent = Intent(this@EventParty, SoloDetails::class.java)
                ...
                filter.setOnClickListener {
                    val intent = Intent(this@EventParty, AdvancedSearch::class.java)
                    intent.putExtra("id", placeid)
                    intent.putExtra("name", model.Name)
                    intent.putExtra("status", model.Status)
                    intent.putExtra("type", model.Type)
