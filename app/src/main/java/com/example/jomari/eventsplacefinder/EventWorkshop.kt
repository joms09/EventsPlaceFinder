...
class EventWorkshop : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ref = FirebaseDatabase.getInstance().getReference("event").orderByChild("type").equalTo("Workshop")
    ...
    fun firebaseData() {
        ...
        val firebaseRecyclerAdapter =
            object : FirebaseRecyclerAdapter<Model, com.example.jomari.eventsplacefinder.MyViewHolder>(option) {
                override fun onCreateViewHolder(
                    parent: ViewGroup,
                    viewType: Int
                ): com.example.jomari.eventsplacefinder.MyViewHolder {
                    val itemView = LayoutInflater.from(this@EventWorkshop).inflate(R.layout.card_view, parent, false)
                    return com.example.jomari.eventsplacefinder.MyViewHolder(itemView)
                }

                override fun onBindViewHolder(
                    holder: com.example.jomari.eventsplacefinder.MyViewHolder,
                    position: Int,
                    model: Model
                ) {
                    ...
                    holder.img_vet.setOnClickListener {
                        ...
                        event.child("count").setValue(model.Count + 1).addOnCompleteListener {
                            val intent = Intent(this@EventWorkshop, SoloDetails::class.java)
                    ...
                    filter.setOnClickListener {
                        val intent = Intent(this@EventWorkshop, AdvancedSearch::class.java)
                        intent.putExtra("id", placeid)
                        intent.putExtra("name", model.Name)
                        intent.putExtra("status", model.Status)
                        intent.putExtra("type", model.Type)
